...
> 一款超现实叙事游戏。玩家将在「记忆探索 → 潜渊解谜 → 庭审辩论」的循环中揭开"记忆托管公司"的真相。
...
Boot → MainMenu → Cutscene → Memory → Abyss → Court → Result
 │        │          │          │        │        │        │
 │   主菜单/UI   过场动画   记忆探索   潜渊解谜  庭审辩论   结局
 │                                 （接水管，走廊等）  （LLM驱动）
 └─ 自动初始化所有 Manager（RuntimeInitializeOnLoadMethod）
```
...
| 文件 | 职责 |
|------|------|
| `PlayerMovement.cs` | 2D 玩家移动（Input System），支持冻结/解冻 |
| `PlayerInteraction.cs` | 玩家交互检测。通过 Trigger2D 检测附近的 `MemoryNodeBase`，按交互键触发 |
| `PlayerInteractInput.cs` | 交互输入桥接。将 Input System 的 `OnInteract` 回调路由到 `PlayerInteraction` |
| `MemoryFragmentNode.cs` | 记忆碎片节点（继承 `MemoryNodeBase`）。玩家拾取后通知 `AbyssPortal` 计数 |
| `AbyssPortal.cs` | **潜渊传送门**。收集足够碎片后触发渐变黑屏 + 场景切换 |
...
玩家进入 Memory 场景
    │
    ├── PlayerMovement: WASD 移动
    ├── PlayerInteraction: Trigger 检测碎片节点
    │       ↓
    ├── MemoryFragmentNode.Interact() → 显示记忆面板
    ├── MemoryFragmentNode.OnTriggerEnter2D → AbyssPortal.CollectFragment()
    │
    └── 收集完毕 → AbyssPortal 开启
            ↓
        渐变黑屏 → 加载潜渊场景
...
| 文件 | 职责 |
|------|------|
| `ConsensusTitleEffect.cs` | 标题"解码"特效 — 随机字符逐字还原 + 持续 Glitch 抖动 |
| `TitleInteraction.cs` | 标题悬停交互 — 鼠标悬停变色 |
| `ButtonHoverEffect.cs` | 按钮悬停 — 文字加框 + 变色（`[ START ]` 风格） |
| `ButtonHover.cs` | 按钮悬停 — 缩放效果（1.1x） |
| `FloatEffect.cs` | UI 浮动动画 — 正弦波上下漂浮 |
| `InteractionPromptUI.cs` | 交互提示 — 靠近可交互物体时显示提示 UI |
